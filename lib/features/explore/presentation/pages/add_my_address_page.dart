...
import '../blocs/user_address_bloc.dart';
    ...
    return ListView.separated(
      ...
      itemBuilder: (_, index) {
        ...
        return ListTile(
          ...
          onTap: () async {
            try {
              final url = Uri.parse('https://rsapi.goong.io/place/detail?place_id=${coordinate['place_id']}&api_key=i9WBxoabU5xQF7ViPKwrEIp5roEEUfs0ZvxBOf7C');
              var response = await http.get(url);
              final jsonResponse = jsonDecode(response.body);
              ...
              final address = AddressModel(id: 0, name: widget.addressName, address: details['name'], province: details['compound']['province'], lat: details['geometry']['location']['lat'], lng: details['geometry']['location']['lng'], commune: details['compound']['commune'], district: details['compound']['district']);

              Navigator.push(context, MaterialPageRoute(builder: (_) => BlocProvider.value(value: context.read<UserAddressBloc>(), child: AddAddressPage(address: address, isUpdate: false,),)));
            } catch (e) {
              SnackBarHelper.showError(e.toString());
            } finally {
              if (!mounted) return;
