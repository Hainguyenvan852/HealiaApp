...
import 'package:healio_app/features/user/auth/domain/usecases/set_role_manager_usacase.dart';
import 'package:healio_app/features/user/profile/data/models/user_model.dart';
import 'package:healio_app/features/user/auth/domain/usecases/facebook_sign_in_usecase.dart';
import 'package:healio_app/features/user/profile/domain/usecases/get_user_info_usecase.dart';
import 'package:healio_app/features/user/auth/domain/usecases/google_sign_in_usecase.dart';
import 'package:healio_app/features/user/auth/domain/usecases/verify_user_account.dart';
...
class AuthBloc extends Bloc<AuthEvent, OAuthState> {
  ...
  final SetRoleManagerUsacase setRoleManagerUsacase;
  ...
  AuthBloc({
    ...
    required this.setRoleManagerUsacase,
  }) : super(AuthInitial()) {
    ...
    on<VerifyEmailRequest>((event, emit) async {
      emit(AuthLoading());
      try {
        final response = await verifyUserAccountUseCase.call(
          event.email,
          event.token,
        );
    ...
    on<ResendTokenRequest>((event, emit) {
      ...
      try {
        ...
        emit(ResendVerificationTokenSuccess());
      } catch (e) {
        emit(AuthError(errorMsg: SupabaseAuthExceptionHandler.parse(e)));
      }
    });
    ...
    on<UserSignedIn>((event, emit) async {
      emit(AuthLoading());
      try {
        await signInUserUseCase.call(event.email, event.password);
      } catch (e) {
        emit(AuthError(errorMsg: SupabaseAuthExceptionHandler.parse(e)));
      }
    });
    ...
    on<GoogleSignInRequested>((event, emit) async {
      ...
      try {
        ...
        //   final currentSession = checkUserSessionUseCase.call();
        //   if (currentSession != null) {
        //     await setRoleManagerUsacase(currentSession.user.id);
        //   }
        // }
      } on AuthException catch (e) {
        emit(
          AuthError(errorMsg: SupabaseAuthExceptionHandler.parse(e.message)),
        );
      } catch (e) {
        emit(AuthError(errorMsg: SupabaseAuthExceptionHandler.parse(e)));
      }
