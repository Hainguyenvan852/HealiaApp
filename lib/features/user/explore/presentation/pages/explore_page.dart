...
import 'package:healio_app/features/user/explore/presentation/widgets/explore_page_shimmer.dart';
import 'package:healio_app/features/user/explore/presentation/widgets/explore_search_bar.dart';
import 'package:healio_app/features/user/explore/presentation/widgets/sliding_store_card.dart';
import 'package:healio_app/features/user/explore/presentation/widgets/sliding_up_header.dart';
import 'package:healio_app/features/user/explore/presentation/widgets/stores_list.dart';
...
class _ExplorePageState extends State<ExplorePage> {
  ...
  bool isProgrammaticMove =
      false; // Cờ đánh dấu tránh việc load 2 lần khi chọn clear

  @override
  void initState() {
    super.initState();
    _lastSearchedLocation = const LatLng(
      21.024536278826037,
      105.83668830463456,
    );
  ...
  void _onMapCreated(MapLibreMapController controller) async {
    ...
    try {
      ...
      final ByteData bytesSelected = await rootBundle.load(
        'assets/icons/convenience-store.png',
      );
      await mapController!.addImage(
        'marker-selected',
        bytesSelected.buffer.asUint8List(),
      );
    } catch (e) {
      print("Lỗi nạp ảnh: $e");
    }
  ...
  Future<void> _onCameraIdle() async {
    ...
    if (distanceInMeters > _distanceThreshold) {
      ...
      if (finalState.category != 'All treatments' &&
          finalState.date != null &&
          finalState.startTime != null &&
          finalState.endTime != null) {
        context.read<EStoreBloc>().add(
          SearchByFilter(
            userLat: finalState.lat!,
            userLng: finalState.lng!,
            radiusKm: 20,
      ...
      } else if (finalState.category == 'All treatments' &&
          finalState.date != null &&
          finalState.startTime != null &&
          finalState.endTime != null) {
        context.read<EStoreBloc>().add(
        ...
        context.read<EStoreBloc>().add(
          ...
          finalState.startTime == null &&
          finalState.endTime == null &&
          finalState.locationName != 'Current location') {
        context.read<EStoreBloc>().add(
          SearchByFilter(
            userLat: finalState.lat!,
            userLng: finalState.lng!,
  ...
  Future<void> _onStyleLoadedCallback(List<StoreModel> stores) async {
    ...
    if (stores.isNotEmpty &&
        context.read<EStoreBloc>().state.searchStores == null) {
  ...
  void _onSearchPressed() async {
    ...
    if (result == 'search') {
      ...
      if (state.lat == null && state.lng == null) {
        final response = await MapHelper.requestLocationPermission();
        if (!response) {
          context.read<SearchFilterCubit>().updateSearch(
            state.copyWith(lng: 105.83668830463456, lat: 21.024536278826037),
          );
        } else {
          ...
          if (lastPosition != null) {
            ...
            context.read<SearchFilterCubit>().updateSearch(
              state.copyWith(lng: position.longitude, lat: position.latitude),
            );
          }
        }
      }
  ...
  Widget build(BuildContext context) {
    ...
    final String styleUrl =
        "https://tiles.goong.io/assets/goong_map_highlight.json?api_key=$tileKey";
      ...
      builder: (context, searchState) {
        return BlocConsumer<EStoreBloc, EStoreState>(
          ...
          listener: (context, state) async {
            ...
            if (!state.isLoading &&
                state.aroundStores.isNotEmpty &&
                state.searchStores == null &&
                !state.isSearching) {
              mapController!.clearSymbols();
              await MapHelper.addStoreMarkers(
                mapController,
                state.aroundStores,
              );
                  ...
                  Positioned.fill(
                    child: MapLibreMap(
                      ...
                      initialCameraPosition: const CameraPosition(
                        target: LatLng(21.024536278826037, 105.83668830463456),
                        zoom: 9,
                      ),
                      ...
                      onStyleLoadedCallback: () =>
                          _onStyleLoadedCallback(state.aroundStores),
                  ...
                  Positioned(
                    ...
                    child: state.isLoading
                        ...
                        : ExploreSearchBar(
                            ...
                            datetime:
                                searchState.dateText +
                                ' - ' +
                                searchState.timeText,
                          ),
                  ),
                          ...
                          child: Transform.translate(
                            ...
                            child: Opacity(
                              ...
                              child: SlidingStoreCard(
                                ...
                                onClose: onCloseCard,
                                onTap: () {
                                  final user = inj<CheckCurrentUserUseCase>()
                                      .call();

                                  context.read<StoreBloc>().add(
                                    AddRecentlyStore(
                                      _selectedSymbol!.data!['store'].id
                                          .toString(),
                                    ),
                                  );
                                  context.read<BookingCubit>().selectStore(
                                    _selectedSymbol!.data!['store'],
                                  );
                                  context
                  ...
                  if (!state.isLoading)
                    Positioned(
                      ...
                      child: Transform.translate(
                        offset: Offset(
                          0,
                          _panelPositionNotifier.value * _hideOffset,
                        ),
                        child: IconButton(
                          onPressed: () async {
                            final status = await MapHelper.checkPermission();
        ...
        child: Center(
          child: Column(
            children: [
              ...
              Text(
                AppLocalizations.of(context)!.tryNewSearch,
                style: GoogleFonts.quicksand(
                  fontWeight: FontWeight.w600,
