...
import 'package:healio_app/features/user/explore/domain/usecases/search_by_all_filter_usecase.dart';
import 'package:healio_app/features/user/explore/domain/usecases/search_by_category_usecase.dart';
import 'package:healio_app/features/user/explore/domain/usecases/search_by_date_usecase.dart';
import 'package:healio_app/features/user/explore/domain/usecases/search_by_datetime_usecase.dart';
import 'package:healio_app/features/user/explore/domain/usecases/search_by_filter_usecase.dart';
import 'package:healio_app/features/user/explore/domain/usecases/search_by_time_usecase.dart';
...
class EStoreBloc extends Bloc<EStoreEvent, EStoreState> {
  ...
  EStoreBloc({
    ...
    on<LoadingEStore>((event, emit) async {
      try {
        emit(state.copyWith(isLoading: true));
        final prefs = await SharedPreferences.getInstance();
        Position? position;

        final isFirstRequest = prefs.getBool('location-request');
        if (isFirstRequest == null || !isFirstRequest) {
          final status = await MapHelper.requestLocationPermission();
          if (status) {
        ...
        if (position != null) {
          aroundStores = await searchStoreAroundLocationUseCase.call(
            position.latitude,
            position.longitude,
            100,
          );
          aroundStores = aroundStores.map((store) {
        ...
        await prefs.setBool('location-request', true);
    ...
    on<SearchByFilter>((event, emit) async {
      try {
        ...
        if (event.category != null &&
            ...
            event.date != null &&
            event.endTime != null) {
          searchStores = await searchByAllFilterUseCase.call(
            event.category!,
            event.startTime!,
            event.endTime!,
            event.date!,
            ...
            event.date == null &&
            event.category == null) {
          searchStores = await searchByFilterUseCase.call(
            event.filter!,
            event.userLat,
            event.userLng,
            event.radiusKm,
            ...
            event.date != null &&
            event.category == null) {
          searchStores = await searchByDateTimeUseCase.call(
            event.startTime!,
            event.endTime!,
            event.date!,
            event.userLat,
            ...
            event.date != null &&
            event.category == null) {
          searchStores = await searchByDateUseCase.call(
            event.date!,
            event.userLat,
            event.userLng,
            event.radiusKm,
            ...
            event.date == null &&
            event.filter == null) {
          searchStores = await searchByCategoryUseCase.call(
            event.category!,
            event.userLat,
            event.userLng,
            event.radiusKm,
            ...
            event.date == null &&
            event.filter == null) {
          searchStores = await searchByTimeUseCase.call(
            event.startTime!,
            event.endTime!,
            event.userLat,
            event.userLng,
            ...
            event.date == null &&
            event.filter == null) {
          searchStores = await searchStoreAroundLocationUseCase.call(
            event.userLat,
            event.userLng,
            event.radiusKm,
          );
